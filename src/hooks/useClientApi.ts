...
export const useClientApi = () => {
  ...
  const getClientById = useCallback(
    async (id: number): Promise<Client | null> => {
      return handleRequest<Client>(() => fetch(`${API_BASE_URL}/users/${id}`));
    },
    [handleRequest]
  );

  const createClient = useCallback(
    async (clientData: ClientFormData): Promise<Client | null> => {
  ...
  const updateClient = useCallback(
    async (id: number, clientData: ClientFormData): Promise<Client | null> => {
      return handleRequest<Client>(() =>
